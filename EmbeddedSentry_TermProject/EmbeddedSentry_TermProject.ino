...
int LEDRIGHT = 3; //Pin for Green LED
int LEDWRONG = 4; //Pin for Red LED
unsigned char showComment; //Flag to prevent comment from generating during each loop
...
void loop() {
  ...
  {
    switch(state){
      // cases 0-3 is Phase 1: "Record Key" feature 
      // cases 4-7 is Phase 2: "Unlocking the Resource" 
      case 0 :
        Serial.println("Hello! You have yet to set a key. Please complete the following prompts to do so");
        Serial.println("---------------------------------------------------------------------------------");
        // printKey(); // for debugging
        state = 1;
        break;
      case 1 :
      ...
      case 4 : // checking for first movement to match first key value
        if (checkingInput(accelVals) == 3){ // no movement detected
          if (showComment == 0){
            Serial.println("Awaiting First Key Value");
            showComment = 1;
          }
          state = 4;
          break;
